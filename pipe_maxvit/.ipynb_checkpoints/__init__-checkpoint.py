...
def window_partition_pipe(
    ...
    if test == True:
        return windows
    return windows.view(-1, grid_window_size[0] * grid_window_size[1], C)
...
def grid_partition_pipe(
    ...
    grid = grid.permute(0, 3, 5, 2, 4, 1).contiguous().view(-1, grid_size[0], grid_size[1], C)
    if test == False:
        return grid.view(-1, grid_size[0] * grid_size[1], C)
    return grid
...
def PipeMaxViTTransformerBlock(in_channels: int, partition_function: Callable, reverse_function: Callable, num_heads: int = 32,
            grid_window_size: Tuple[int, int] = (7, 7), attn_drop: float = 0., drop: float = 0., drop_path: float = 0.,
            mlp_ratio: float = 4., act_layer: Type[nn.Module] = nn.GELU, norm_layer: Type[nn.Module] = nn.LayerNorm,):
    partition_function: Callable = partition_function
    reverse_function: Callable = reverse_function
    grid_window_size: Tuple[int, int] = grid_window_size
    norm_1 = norm_layer(in_channels)
    ...
    input_partitioned = partition_function(input, grid_window_size)
...
def PipeMBConv(in_channels: int, out_channels: int, downscale: bool = False, act_layer: Type[nn.Module] = nn.GELU,
            norm_layer: Type[nn.Module] = nn.BatchNorm2d, drop_path: float = 0.):
    ...
    skip_path = SkipConnection(in_channels, out_channels)
    # Make skip path
...
# my custom functions start from this line
class CustomPool(nn.Module):
    def __init__(self, global_pool="avg"):
        super(CustomPool, self).__init__()
        self.global_pool = global_pool

    def forward(self, input: torch.Tensor, pre_logits: bool = False):
...
def PipeMaxViTBlock(in_channels: int, out_channels: int, downscale: bool = False, num_heads: int = 32,
            grid_window_size: Tuple[int, int] = (7, 7), attn_drop: float = 0., drop: float = 0., drop_path: float = 0.,
            mlp_ratio: float = 4., act_layer: Type[nn.Module] = nn.GELU, norm_layer: Type[nn.Module] = nn.BatchNorm2d,
            norm_layer_transformer: Type[nn.Module] = nn.LayerNorm):
    ...
    # Init Block and Grid Transformer
    block_transformer = MaxViTTransformerBlock(
        in_channels=out_channels,
        partition_function=window_partition,
        reverse_function=window_reverse,
        num_heads=num_heads,
...
def PipeMaxViTStage(depth, in_channels, out_channels, num_heads, grid_window_size, attn_drop, drop, drop_path,
                    mlp_ratio, act_layer, norm_layer, norm_layer_transformer):
    """ Stage of the MaxViT.
    Args:
        depth (int): Depth of the stage.
        in_channels (int): Number of input channels.
...
def maxvit(in_channels: int = 3,
    ...
    max_pool = CustomPool()
    head = nn.Linear(channels[-1], num_classes)
